...
class IKTemporalJointReference(ik.IKObjective):
    ...
    def set_target(self, target_q_np):
        """Host-side reference update; broadcasts a single configuration to all problems."""
        if self.target_q is None:
            return
        target = np.asarray(target_q_np, dtype=np.float32)
        if target.ndim == 1:
            target = np.tile(target[None, :], (self.target_q.shape[0], 1))
...
@wp.kernel
def _temporal_reference_residuals_coord_indexed(
...
@wp.kernel
def _temporal_reference_jac_analytic_coord_indexed(
    coord_masks: wp.array1d(dtype=wp.float32),
    n_coords: int,
    start_idx: int,
    weight: wp.array1d(dtype=wp.float32),
...
class IKTemporalJointReferenceLegacy(ik.IKObjective):
    """Penalize selected joint coordinates against an explicit q reference.

    Verbatim ROBOTIS AI Sapiens implementation, preserved for fidelity: its
    analytic Jacobian indexes DOF columns by coordinate index, which is offset
    by one after the free joint. That is a bug, but the shipped AI Sapiens
    configs were tuned against this behavior - fixing it moves K1 outputs
    well outside the pipeline's own reproducibility envelope. New code should
    use IKTemporalJointReference (correct DOF mapping, per-problem weights).
    """
    ...
    def compute_residuals(self, body_q, joint_q, model, residuals, start_idx, problem_idx):
        count = joint_q.shape[0]
        wp.launch(
            _temporal_reference_residuals_coord_indexed,
    ...
    def compute_jacobian_analytic(self, body_q, joint_q, model, jacobian, joint_S_s, start_idx):
        wp.launch(
            _temporal_reference_jac_analytic_coord_indexed,
