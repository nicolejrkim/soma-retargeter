...
from soma_retargeter.pipelines.ik_objectives import (
    ...
    IKTemporalJointReferenceLegacy,
)
...
class NewtonPipeline:
    ...
    def __init__(self, skeleton: Skeleton, source_type='soma', robot_type='unitree_g1', retarget_config: dict = None):
        ...
        if True:  # robot model resolved via the robot registry
            ...
            # decompositions on a consistent branch (same feature as retargeter-suite).
            ...
            # reachable sphere before the IK ever sees them (same feature as
            # retargeter-suite).
            ...
            if limb_reach_projection is not None:
                ...
                _lr_state = self.ik_model.state()
                newton.eval_fk(self.ik_model, self.ik_model.joint_q, self.ik_model.joint_qd, _lr_state)
                _lr_body_pos = _lr_state.body_q.numpy()[:, :3]
                for chain in limb_reach_projection.get('chains', []):
                    ...
                    upper_len = float(np.linalg.norm(_lr_body_pos[mid_b] - _lr_body_pos[root_b]))
                    lower_len = float(np.linalg.norm(_lr_body_pos[tip_b] - _lr_body_pos[mid_b]))
    ...
    def execute(self):
        ...
        if temporal_reference_objective is not None and self.temporal_reference_boost:
            ...
            print(f"[INFO]\t  Temporal Reference Boost: {int(temporal_boost_windows.sum())} frame-envs below {threshold} m")
    ...
    def _create_ik_objectives(self, num_envs, model, state):
        ...
        neutral_reference_objective = IKTemporalJointReferenceLegacy(
        ...
        temporal_yaw_twist_reference_objective = IKTemporalJointReferenceLegacy(
            joint_coord_count=self.ik_model.joint_coord_count,
